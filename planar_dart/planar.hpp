...
namespace planar_dart {
    ...
    public:
        ...
        planar(const std::string& model_file, const std::string& robot_name, const std::vector<std::pair<std::string, std::string>>& packages, bool is_urdf_string, std::vector<planarDamage> damages) : _robot_name(robot_name), _skeleton(_load_model(robot_name, model_file, packages, is_urdf_string))
        {
        ...
        planar(dart::dynamics::SkeletonPtr skeleton, std::vector<planarDamage> damages) : _skeleton(skeleton)
        {
            assert(_skeleton != nullptr);
            _set_damages(damages);
        ...
        std::shared_ptr<planar> clone() const
        {
            ...
            auto planar = std::make_shared<planar>();
            planar->_skeleton = tmp_skel;
            planar->_damages = _damages;
            planar->_stuck_joints = _stuck_joints;
            return planar;
        }
