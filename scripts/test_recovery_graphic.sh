...
for method in $1; do
    for test_type in $2; do
        ...
        Outfolder=~/ToyData/Replicate3/${method_tag}/exp3 # destination folder
        echo "will write to ${Outfolder}"

        ${SFERES_DIR}/build/exp/MAP-Elites-Rhex/test_${binary_tag}_binary --load ${Outfolder}/gen_${last_gen} --d ${Outfolder} -o ${Outfolder}/${test_type}_performanceGRAPHDUMMY -n $individual
    done
